...
function get20s(array) {
  ...
  for(let i = 0; i < array.length; i++){
    if(array[i].years > "1904"){
      longGone.push(artists[i].name);
    }
  }
  return longGone;
}
...
function addArtist(array) {
  array.push({ 
    ...
    bio: "Looking forward to this new line of allowing me the opportunity to get out in the world to make more art "
  }  )
  return array;
}
...
function lotsOfArt(array) {
  ...
  for(let i = 0; i < array.length; i++){
    if(array[i].paintings > 100){
      hardWorkPays.push(artists[i].name);
    }
  }
  return hardWorkPays;
}
...
console.log(artistByCountry(artists, "Flemish"));
